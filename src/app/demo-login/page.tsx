/*
|-----------------------------------------
| setting up demo-login/page.tsx for the App
| @author: Toufiquer Rahman<toufiquer.0@gmail.com>
| @copyright: Toufiquer, 24 September 2026
|-----------------------------------------
*/
...
export default function DemoLoginPage() {
  ...
  function selectDemoAccount(accountEmail: string) {
    ...
    const account = demoAccounts.find((demoAccount) => demoAccount.email === accountEmail);
    if (!account) return;
      ...
      <div className="relative mx-auto grid w-full max-w-5xl overflow-hidden rounded-sm border border-[#eadfca] bg-white shadow-[0_24px_80px_-34px_rgba(120,53,15,0.20)] lg:min-h-[650px] lg:grid-cols-[0.9fr_1.1fr]">
        ...
        <section className="flex items-center px-5 py-8 sm:px-10 sm:py-12 lg:px-14">
          <div className="mx-auto w-full max-w-md">
            <h1 className="text-3xl font-semibold tracking-tight text-stone-900 sm:text-4xl">Demo sign in</h1>
            <button
            ...
            <form className="space-y-4" onSubmit={signIn}>
              <label className="block text-sm font-medium text-stone-700">
                <span className="mb-1.5 block">Demo account</span>
                <select
                  aria-label="Demo account"
                  className="w-full rounded-sm border border-stone-200 bg-white px-4 py-3 text-sm text-stone-900 outline-none transition focus:border-amber-600 focus:ring-4 focus:ring-amber-500/10"
                  disabled={submitting}
